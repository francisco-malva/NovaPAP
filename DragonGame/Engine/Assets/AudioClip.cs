...
public class AudioClip : IDisposable
{
    ...
    ///     Is the file being streamed? If not, the PCM data will be copied to memory completely before
    ///     being read, occupying more memory but being faster. Use streaming only with long files (for example, music tracks).
    /// </param>
    ...
    public void Dispose()
    {
        ...
        Stream?.Dispose();
        if (_streaming) File.Delete(_tempFilePath);
    }
}