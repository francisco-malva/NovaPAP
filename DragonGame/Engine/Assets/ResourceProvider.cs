...
internal abstract class ResourceProvider<T> : IDisposable where T : IDisposable
{
    /// <summary>
        ...
        get
        {
            ...
            if (!File.Exists(path))
                throw new FileNotFoundException(
                    $"Could not load {typeof(T).Name} because '{_rootPath} points to a non existent file.'");
