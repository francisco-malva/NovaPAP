...
async void GetScores(Socket sock)
{
    ...
    command.CommandText = "SELECT Name, Time FROM Times ORDER BY Time LIMIT 10";
...
async void UpdateScores(string name, int time)
{
    ...
    command.CommandText = "INSERT INTO Times VALUES (NULL, @name, @time)";

    command.Parameters.Add("@name", DbType.String).Value = name;
    command.Parameters.Add("@time", DbType.Int32).Value = time;
